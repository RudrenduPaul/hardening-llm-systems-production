...
| [`ch08_scripts.py`](ch08_scripts.py) | `build_analyzer` / `analyze_text` (Listing 8.1), `build_anonymizer` / `anonymize_text` (Listing 8.2), `PIIGuardedLLMPipeline` (Listing 8.3), `QuasiIdentifierTracker` (Listing 8.3a), `memorization_probe` (Listing 8.4), `scan_reasoning_model_output` (Listing 8.5), `scan_o3_response` (Listing 8.5a), `CoTFilter` (Listing 8.5b), `execute_right_to_erasure` (Listing 8.6), `pgvector_erasure` / `weaviate_erasure` (Listing 8.6a), `ErasureLedger` (Listing 8.6b), `PIIGateConfig` / `run_pii_gate` (Listing 8.7) |
...
- **`QuasiIdentifierTracker`** (Listing 8.3a) — session-level combination scoring for quasi-identifiers; fires `HIGH` risk when 3+ distinct quasi-id types reference the same named entity
...
- **`scan_o3_response`** (Listing 8.5a) — dual-output PII scanner for OpenAI o3/o4-mini via the responses API; flags PII present in the reasoning summary but not the final output
- **`CoTFilter`** (Listing 8.5b) — scans chain-of-thought traces for system-prompt-leak signals and Presidio PII before the trace reaches the user; returns a sanitized trace or full suppression
...
- **`pgvector_erasure` / `weaviate_erasure`** (Listing 8.6a) — right-to-erasure implementations for pgvector (`DELETE ... WHERE user_id`) and Weaviate (`delete_many` with a property filter)
- **`ErasureLedger`** (Listing 8.6b) — JSON-persisted `doc_id -> embedding_ids` ledger implementing the two-phase erasure protocol; `execute_erasure()` returns a GDPR Article 5(2) `ErasureReport`
...
> **No API keys required for most demos.** `build_analyzer()` / `build_anonymizer()` degrade to `None` and log a warning if `presidio-analyzer` / `presidio-anonymizer` aren't installed, and `PIIGuardedLLMPipeline` / `memorization_probe` accept any injected client, so the notebook exercises them with a stub client. `scan_reasoning_model_output()` and `scan_o3_response()` instantiate a live `anthropic.Anthropic()` / `openai.OpenAI()` client internally, but the notebook runs those cells against monkeypatched fake clients by default, so no `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` is needed to execute them; swap in a real client to hit the live APIs.
